...
class Display(CharLCD):
    ...
    def initialize(self):
        try:
            ...
            self.has_hardware_error = False
            self.last_event_time = None
            self.loading()
            self.render()
            self.done_loading()
        except OSError as e:
            print("Failed to initialize LCD hardware")
    ...
    def write_line(self, text, line=0):
        if self.has_hardware_error:
            print("Attempting to reinitialize display")
            self.initialize()
        try:
            self.cursor_pos = (line, 0)
            self.write_string(text)
        except OSError as e:
            self.has_hardware_error = True